...
# # 方案1：临时屏蔽（仅本次运行有效，推荐）
# CUDA_VISIBLE_DEVICES=0 python ./train/train__0306.py

# # 方案2：终端会话内永久屏蔽（本次登录服务器都生效）
# export CUDA_VISIBLE_DEVICES=0  # 执行一次即可
# python ./train/train_0306.py  # 后续运行都只用cuda:0
import sys
import os
import socket
import setproctitle
...
from config_0306 import get_config
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            from envs.env_continuous0306 import ContinuousActionEnv
...
def parse_args(args, parser):
    ...
    parser.add_argument("--num_agents", type=int, default=13, help="number of players")  # <--- 改成 13#智能体个数
...
def main(args):
    ...
    if not run_dir.exists():
        ...
        exst_run_nums = [
            int(str(folder.name).split("_0306_run")[1])
            for folder in run_dir.iterdir()
            if str(folder.name).startswith("_0306_run")
        ]
        if len(exst_run_nums) == 0:
            curr_run = "_0306_run1"
        else:
            curr_run = "_0306_run%i" % (max(exst_run_nums) + 1)
    ...
    if all_args.share_policy:
        ...
        from runner.separated.env_runner0306 import EnvRunner as Runner
